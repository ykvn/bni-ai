...
try:
    ...
    print("⚠️ sqlglot not installed. Falling back to basic word intersection.")
    ...
    try:
        ...
        if isinstance(data, dict) and "tables" in data:
            for table in data["tables"]:
                ...
                for col in table.get("columns", []): col.pop("score", None)
...
def _normalize_table_dict(table: dict, columns: list) -> dict:
    clean_cols = []
    for col in columns:
        clean_col = {k: v for k, v in col.items() if k in ["name", "score", "type", "references", "description"]}
...
def extract_sql_metadata(text: str, valid_tables: set, valid_columns: set) -> tuple[set, set]:
    found_tables, found_columns = set(), set()
    if SQLGLOT_AVAILABLE:
        try:
            sql_blocks = re.split(r"(?is)\s*SQL:\s*", text)[1:] if "SQL:" in text else [text]
            for block in sql_blocks:
                ...
                for stmt in sqlglot.parse(block, read="impala"):
                    ...
                    for col in stmt.find_all(exp.Column):
                        if col.name and col.name.lower() in valid_columns: 
                            found_columns.add(col.name.lower())
            if found_tables or found_columns: return found_tables, found_columns
    ...
    return words.intersection(valid_tables), words.intersection(valid_columns)

def get_smart_schema_context_with_golden(
    user_query: str, top_tables: int = 5, top_columns_per_table: int = 10,
    relative_threshold_ratio: float = 0.05, absolute_min_score: float = 0.20,
    ...
    try:
        ...
        if retrieved_points and "error" in retrieved_points[0]: raise RuntimeError(retrieved_points[0]["error"])
    except Exception as e:
        return yaml.dump({"database_type": "Cloudera Impala", "error": f"CRITICAL ERROR: {e}"}, sort_keys=False), ""

    retrieved_tables, valid_schema_tables, valid_schema_columns = [], set(), set()
    for point in retrieved_points:
        payload = point.get("payload", {})
        if raw_yaml_str := payload.get("raw_yaml"):
            parsed_table = yaml.safe_load(raw_yaml_str)
            if point.get("score") is not None: parsed_table["score"] = round(point.get("score"), 4)
            retrieved_tables.append(parsed_table)
            if "name" in parsed_table: valid_schema_tables.add(str(parsed_table["name"]).lower())
            for col in parsed_table.get("columns", []):
                if "name" in col: valid_schema_columns.add(str(col["name"]).lower())

    golden_context, golden_query_tables, golden_query_cols = "", set(), set()
    try:
    ...
    except Exception as e: print(f"⚠️ Golden Query extraction warning: {e}")

    for point in retrieved_points:
        for val in point.get("payload", {}).values():
            if any(kw in str(val).upper() for kw in ["SELECT", "FROM", "WHERE"]):
                g_tables, g_cols = extract_sql_metadata(str(val), valid_schema_tables, valid_schema_columns)
                golden_query_tables.update(g_tables)
                golden_query_cols.update(g_cols)
    ...
    for table in retrieved_tables:
        ...
        for col in table.get("columns", []):
            ...
            table_mapping.append({"table": t_name, "column": c_name, "description": clean_desc, "vector_score": table_vector_score})

        if not table_docs: continue

        try:
            rerank_results = rerank_documents(query=user_query, documents=table_docs, engine_url=embed_url, cml_token=cml_token, top_n=len(table_docs), timeout=15)
            boosted_hits = []
            for hit in rerank_results:
                if (idx := hit.get("index")) is not None:
            ...
            relative_threshold = max((top_table_hits[0]["score"] if top_table_hits else 0.0) * relative_threshold_ratio, absolute_min_score)

            for col_data in top_table_hits:
                if col_data["score"] >= relative_threshold: winning_columns.append(col_data)
        except Exception as e: print(f"⚠️ Per-table reranking failed for {t_name}: {e}")

    global_max_score = max([item['score'] for item in winning_columns], default=0.0)
    
    # 🌟 NOISE GUARD: Reject query if the top winning column fails absolute minimum confidence
    if global_max_score < absolute_min_score:
        print(f"🚫 [Noise Guard] Top score ({global_max_score}) below floor ({absolute_min_score}). Refusing.")
        return yaml.dump({"database_type": "Cloudera Impala", "tables": [], "error": NO_RELEVANT_SCHEMA}, sort_keys=False), golden_context
    ...
    pruned_tables = []
    for table in retrieved_tables:
        if (table_name := table.get("name")) in relevant_table_names:
            ...
            for col in table.get("columns", []):
                ...
                elif col.get("primary_key") or "references" in col or "DEFAULT_TIME_AXIS: True" in str(col.get("description", "")):
                    mandatory_columns.append(col)
    ...
    return yaml.dump({"database_type": "Cloudera Impala", "tables": pruned_tables}, sort_keys=False, default_flow_style=False), golden_context

def _has_no_tables(schema_context: str) -> bool:
    if NO_RELEVANT_SCHEMA in schema_context: return True
    try:
        tables = yaml.safe_load(schema_context).get("tables")
...
def search_database_schema(user_question: str) -> str:
    ...
    if _has_no_tables(raw_schema_context): return NO_SCHEMA_RESPONSE
    return f"{raw_schema_context}\n\n{golden_context}" if golden_context and "No verified golden" not in golden_context else raw_schema_context
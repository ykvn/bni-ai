...
class SQLTranslationService:
    ...
    def _extract_sql_from_response(ai_result: str) -> str:
        """Extracts SQL from markdown blocks or falls back to raw text."""
        ai_result = str(ai_result).strip()
        
        # 1. Try to strictly find text inside ```sql ... ``` or ``` ... ```
        import re
        ...
        # 2. If the agent forgot markdown wrappers but wrote a SELECT/WITH statement, 
        # clean it up and return it directly.
        if "SELECT" in ai_result.upper() or "WITH" in ai_result.upper():
            return ai_result.replace("```", "").strip()
        
        # 3. If it completely failed to write SQL (e.g., output pure natural language),
        # return exactly what the agent said so you can see it in the UI for debugging!
        return ai_result
